...
class DataFusionEngine:
    ...
    def simulate_comprehensive_data(self):
        print("Generating comprehensive simulated data...")
        ...
        for date in dates:
            for state in states:
                data.append({
                    'date': date.strftime('%Y-%m-%d'),
                    'state': state,
                    'cases_reported': max(0, int(np.random.poisson(50))),
                    'tweet_count': max(0, int(np.random.poisson(1000))),
                    'temperature': round(np.random.normal(70, 10), 1),
